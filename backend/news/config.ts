// Environment configuration for the UNSPIN system
...
export const CONFIG = {
  ...
  // External services
  GUARDIAN_API_KEY: process.env.GUARDIAN_API_KEY || "", // Optional
  NYT_API_KEY: process.env.NYT_API_KEY || "", // Optional
  
  // Logging
  LOG_LEVEL: process.env.LOG_LEVEL || "info",
  LOG_EXTRACTION_DETAILS: process.env.LOG_EXTRACTION_DETAILS === "true", // Default false
  LOG_LLM_RESPONSES: process.env.LOG_LLM_RESPONSES === "true", // Default false
};
...
export function validateConfig(): string[] {
  ...
  if (CONFIG.CACHE_TTL_MS < 0 || CONFIG.CACHE_TTL_MS > 86400000) { // Max 24 hours
  ...
  if (!["openai,gemini", "gemini,openai"].includes(CONFIG.LLM_ORDER)) {
    errors.push("LLM_ORDER must be 'openai,gemini' or 'gemini,openai'");
  }
